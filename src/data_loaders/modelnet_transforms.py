    ...
    def __call__(self, sample):
        ...
        if 'points' in sample:
            ...
            if 'crop_proportion' not in sample:
                ...
                ref_size = self.num
            elif len(sample['crop_proportion']) == 2:       # enter
                # src_size = math.ceil(sample['crop_proportion'][0] * self.num)
            ...
            # print(np.max(sample['correspondences'][0]),'\t',np.max(sample['correspondences'][1]))
            # print(np.min(sample['correspondences'][0]),'\t',np.min(sample['correspondences'][1]))
        return sample
...
class RandomCrop:
    ...
    def crop(points, p_keep):
        rand_xyz = uniform_2_sphere()       # r or o?
    ...
    def __call__(self, sample):
        ...
        if 'deterministic' in sample and sample['deterministic']:   # enter if test
            np.random.seed(sample['idx'])
        ...
        if len(self.p_keep) == 1:
            ...
            points_src, src_mask = self.crop(sample['points_src'], self.p_keep[0])      # （1433，3），（2048，）
