...
class PreprocessorGPU(torch.nn.Module):
    ...
    def forward(self, pts: List[torch.Tensor]):
        ...
        batched_points = torch.cat(pts, dim=0)          # cat pts(717*batch_size*2,3)
        ...
        # sampling: ball_query
        data = {        # all_list
            'points': input_points,     # (batched_points_num, 3)
            'neighbors': input_neighbors,       # (batched_points_num, neighbor_limit(50))
            'pools': input_pools,       # (subsampling_batched_pts, neighbor_limit)     # subsampling from batched_points
            'upsamples': input_upsamples,       # (upsampling_batched_pts, neighbor_limit)      # upsampling from batched_points
            'stack_lengths': input_batch_lens,      # (pts_len)
        }
...
def compute_overlaps(batch):
    ...
    overlaps = batch['src_overlap'] + batch['tgt_overlap']      # gt_overlap
    kpconv_meta = batch['kpconv_meta']
    n_pyr = len(kpconv_meta['points'])

    overlap_pyr = {'pyr_0': torch.cat(overlaps, dim=0).type(torch.float)}       # gt_overlap
    invalid_indices = [s.sum() for s in kpconv_meta['stack_lengths']]
    for p in range(1, n_pyr):
        pooling_indices = kpconv_meta['pools'][p - 1].clone()       # down_sampling patches
