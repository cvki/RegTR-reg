...
from src.models.scheduler.warmup import WarmUpScheduler
from src.benchmark.benchmark_predator import benchmark as benchmark_predator
import src.benchmark.benchmark_modelnet as benchmark_modelnet
from src.utils.misc import StatsMeter, metrics_to_string
from src.utils.se3_torch import se3_compare
...
class GenericRegModel(GenericModel, ABC):
    ...
    def __init__(self, cfg, *args, **kwargs):
        ...
        self.reg_success_thresh_rot = cfg.model.reg_success_thresh_rot
        self.reg_success_thresh_trans = cfg.model.reg_success_thresh_trans

    def configure_optimizers(self):  # override

        scheduler_type = self.cfg.solver.get('scheduler', None)
        ...
        # Create optimizer
        if self.cfg.solver.optimizer == 'AdamW':
        ...
        if scheduler_type == 'warmup':
            ...
            self.scheduler = WarmUpScheduler(self.optimizer, self.cfg.solver.scheduler_param, self.cfg.solver.base_lr)
        elif scheduler_type == 'step':
            # Step decay
            self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, self.cfg.solver.scheduler_param[0],
                                                             self.cfg.solver.scheduler_param[1])
